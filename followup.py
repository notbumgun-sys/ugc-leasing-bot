...
def build_draft(name: str, examples: str, experience: str) -> str:
    """Честный шаблон follow-up без сомнительных персональных зацепок.

    Если не можем реально и внимательно разобрать портфолио кандидата, не
    имитируем просмотр. Лучше сразу предложить оплачиваемое тестовое и спокойно
    объяснить, зачем оно нужно.
    """
    name_line = (name or "").strip() or "Привет"
    return (
        f"{name_line}, привет!\n\n"
        "Спасибо за отклик на роль UGC-креатора для ЛизингСток.\n\n"
        "Хотим предложить короткое оплачиваемое тестовое задание: видео 15–30 секунд по нашему ТЗ.\n\n"
        "Тест нужен не для публикации, а чтобы понять, насколько вам подходит наш формат: "
        "живая подача, понятный хук и простая идея без сложного продакшна.\n\n"
        "Оплата за тестовое — 700 ₽. Мы не будем публиковать или использовать видео без вашего отдельного согласия.\n\n"
        "Готовы попробовать?"
    )
